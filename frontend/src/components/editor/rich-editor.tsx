...
import { useUploadImageMutation } from '@/integrations/hardware/rtk/endpoints/misc.endpoints'
...
export default function RichEditor({
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        attributes: {
          class:
            'prose prose-sm sm:prose lg:prose-lg xl:prose-2xl mx-auto focus:outline-none min-h-[400px] p-6 border rounded-lg',
        },
      },
    },
    [],
  ...
  // Dışarıdan gelen content değişirse editor içeriğini güncelle
  useEffect(() => {
    if (!editor) return
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      // Backend: { success: boolean, url: string, ... } gibi dönüyor varsaydım
      ...
      editor
        .chain()
        .focus()
        .setImage({ src: String(result.url), alt: imageAlt || file.name })
        .run()
  ...
  const addLink = () => {
    ...
    if (linkText) {
      editor
        .chain()
        .focus()
        .insertContent(`<a href="${safeUrl}">${linkText}</a>`)
        .run()
    } else {
