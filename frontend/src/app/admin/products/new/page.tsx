...
interface ProductFormState {
  ...
  model: string;
  description: string;
  categoryId: string; // string, Select ile uyumlu
  price: string;
  releaseYear: string;
  imageUrl: string;
}
...
export default function AdminProductsNewPage() {
  ...
  const [affiliateLinks, setAffiliateLinks] = useState<AffiliateLinkForm[]>([
  ...
  if (Array.isArray(tagsResult)) {
    tags = tagsResult;
  } else if (tagsResult && "results" in tagsResult) {
    tags = (tagsResult as PaginatedResult<TagDto>).results;
  }

  const hasGlobalError = categoriesError || tagsError;
  ...
  const updateAffiliateLink = <K extends keyof AffiliateLinkForm>(
    index: number,
    field: K,
    value: AffiliateLinkForm[K],
  ) => {
    setAffiliateLinks((prev) =>
      prev.map((link, i) =>
        i === index
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Django ProductWritePayload + FormData formatına göre
      ...
      // Eğer URL üzerinden kapak resmi kullanmak istersen
      if (formData.imageUrl) {
        formDataToSend.append("cover_image", formData.imageUrl);
      }

      // Dosya olarak kapak resmi
      if (imageFile) {
      ...
      const validLinks = affiliateLinks.filter(
        (link) =>
          link.merchant.trim() && link.urlTemplate.trim(),
      );
      ...
      <div className="mb-8">
        ...
        {hasGlobalError && (
          <div className="mt-2 p-3 rounded-md bg-red-50 border border-red-200 text-sm text-red-700">
            Kategori veya etiket bilgileri yüklenirken bir hata
            oluştu.
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={isCreating}>
            ...
            {isCreating ? "Kaydediliyor..." : "Ürünü Kaydet"}
          </Button>
        </div>
      </form>
    </div>
