...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'email', 'first_name', 'last_name', 'name', 'role', 'avatar', 'bio', 'status', 'email_verified', 'authored_articles_count', 'comments_count', 'created_at', 'updated_at', 'date_joined']
...
class UserSearchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'email', 'first_name', 'last_name', 'name', 'role', 'avatar']
...
class PriceHistorySerializer(serializers.ModelSerializer):
    class Meta:
        model = PriceHistory
        fields = ['id', 'price', 'currency', 'source', 'url', 'recorded_at', 'created_at']
...
class SettingSerializer(serializers.ModelSerializer):
    class Meta:
        model = Setting
        fields = ['id', 'key', 'value', 'description', 'category', 'is_file', 'created_at', 'updated_at']
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Category
        fields = ['id', 'name', 'slug', 'description', 'icon', 'color', 'is_active', 'sort_order', 'parent', 'children', 'article_count', 'product_count', 'created_at']
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    affiliate_links_data = serializers.ListField(write_only=True, required=False)
    ...
    class Meta:
        model = Product
        fields = ['id', 'brand', 'model', 'slug', 'specs', 'price', 'release_year', 'cover_image', 'cover_image_file', 'description', 'category', 'category_id', 'product_specs', 'affiliate_links', 'affiliate_links_data', 'user_reviews', 'review_count', 'average_rating', 'price_history', 'product_tags', 'tags', 'created_at']
        read_only_fields = ['id', 'created_at']
        extra_kwargs = {
    ...
    def create(self, validated_data):
        ...
        print(f"Django Serializer - Specs in validated_data: {validated_data.get('specs', 'NOT_FOUND')}")
        print(f"Django Serializer - Price in validated_data: {validated_data.get('price', 'NOT_FOUND')}")
        print(f"Django Serializer - Affiliate links data: {validated_data.get('affiliate_links_data', 'NOT_FOUND')}")
        
        # Generate unique slug from brand and model if not provided
        if 'slug' not in validated_data or not validated_data['slug']:
            brand = validated_data.get('brand', '').lower()
            model = validated_data.get('model', '').lower()
            ...
            for tr_char, en_char in turkish_chars.items():
                brand = brand.replace(tr_char, en_char)
                model = model.replace(tr_char, en_char)
        ...
        if hasattr(self, 'initial_data') and self.initial_data:
            ...
            print(f"Django Serializer - Processed affiliate_links_data: {affiliate_links_data}")
            print(f"Django Serializer - Processed specs_data: {specs_data}")
        ...
        print(f"Django Serializer - Created product: {product.id}, specs: {product.specs}, price: {product.price}")
        
        # Create ProductSpec objects if specs provided
        if specs_data:
            from .models_extra import ProductSpec
            ...
            print(f"Django Serializer - Created {len(specs_data)} ProductSpec objects")
        ...
        print(f"Django Serializer - Final product specs: {product.specs}, price: {product.price}")
        return product
    ...
    def update(self, instance, validated_data):
        ...
        print(f"=== AFFILIATE LINKS DEBUG ===")
        print(f"Raw affiliate_links_data: {affiliate_links_data}")
        print(f"Type: {type(affiliate_links_data)}")
        
        # Handle Django QueryDict format for affiliate_links_data
        if hasattr(self, 'initial_data') and 'affiliate_links_data' in self.initial_data:
            # Parse Django QueryDict format: affiliate_links_data[0][merchant], affiliate_links_data[0][url_template], etc.
            raw_data = self.initial_data
            affiliate_links_data = []
            
            # Find all affiliate_links_data keys
            affiliate_keys = [key for key in raw_data.keys() if key.startswith('affiliate_links_data[')]
            
            if affiliate_keys:
                # Group by index
                links_by_index = {}
                for key in affiliate_keys:
                    # Extract index and field name from key like "affiliate_links_data[0][merchant]"
                    import re
                    match = re.match(r'affiliate_links_data\[(\d+)\]\[(\w+)\]', key)
                    if match:
                        ...
                        if isinstance(value, list):
                            value = value[0]  # Django QueryDict returns lists
        ...
        # Handle MultiValueDict format (Django's automatic parsing)
        if isinstance(affiliate_links_data, list) and len(affiliate_links_data) > 0:
            if hasattr(affiliate_links_data[0], 'get'):  # Check if it's a MultiValueDict
                # Convert MultiValueDict to regular dict
                processed_links = []
                for link_dict in affiliate_links_data:
                    processed_link = {}
                    for key, value in link_dict.items():
                        # Remove brackets from keys like '[merchant]' -> 'merchant'
                        clean_key = key.strip('[]')
        ...
        # Handle Django QueryDict format for specs_data
        if hasattr(self, 'initial_data') and any(key.startswith('specs[') for key in self.initial_data.keys()):
            # Parse Django QueryDict format: specs[0][name], specs[0][value], etc.
            raw_data = self.initial_data
            specs_data = []
            ...
            if specs_keys:
                ...
                for key in specs_keys:
                    # Extract index and field name from key like "specs[0][name]"
                    import re
        ...
        # Handle clearing cover_image when empty string is provided
        ...
        # Update product tags - always process tags field
        if tags_data is not None:
            from .models_extra import ProductTag
            ProductTag.objects.filter(product=product).delete()
            
            # Handle empty string case (clear all tags)
        ...
        # Update affiliate links - only if data is provided
        from .models import AffiliateLink
        
        # Only update affiliate links if data is provided
        if affiliate_links_data is not None:
            ...
            if isinstance(affiliate_links_data, str) and affiliate_links_data == '[]':
                pass  # All links already cleared
            elif isinstance(affiliate_links_data, list) and len(affiliate_links_data) == 0:
                pass  # All links already cleared
            else:
                for link_data in affiliate_links_data:
                    # Convert string 'true'/'false' to boolean
    ...
    def get_affiliate_links(self, obj):
        return [{'id': str(link.id), 'merchant': link.merchant, 'url_template': link.url_template, 'active': link.active} for link in obj.affiliate_links.all()]
    
    def get_specs(self, obj):
        # specs field'ını product_specs'ten doldur
        if hasattr(obj, 'product_specs') and obj.product_specs.exists():
            return [{'name': spec.name, 'value': spec.value, 'unit': spec.unit or '', 'sort_order': spec.sort_order} for spec in obj.product_specs.all()]
        return []
    ...
    def get_cover_image(self, obj):
        if obj.cover_image:
            ...
            # Değilse Django media URL'i ile birleştir
            from django.conf import settings
            return f"{settings.MEDIA_URL}{obj.cover_image}"
        return None
    ...
    def get_price_history(self, obj):
        price_histories = obj.price_history.all()[:10]  # Son 10 fiyat geçmişi
        return PriceHistorySerializer(price_histories, many=True).data

    def get_product_tags(self, obj):
        return [{'id': tag.tag.id, 'name': tag.tag.name, 'slug': tag.tag.slug, 'type': tag.tag.type} for tag in obj.product_tags.all()]
...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Article
        fields = ['id', 'type', 'slug', 'title', 'subtitle', 'excerpt', 'content', 'status', 'author', 'category', 'category_id', 'published_at', 'hero_image', 'hero_image_file', 'og_image', 'meta_title', 'meta_description', 'view_count', 'article_tags', 'review_extra', 'review_extra_data', 'best_list_extra', 'best_list_extra_data', 'compare_extra', 'comment_count', 'created_at']
        read_only_fields = ['id', 'created_at']
    ...
    def create(self, validated_data):
        ...
        if 'title' in validated_data:
            import re
            base_slug = re.sub(r'[^a-z0-9\s-]', '', validated_data['title'].lower())
        ...
        # Remove extra data fields from validated_data as they're not model fields
    ...
    def update(self, instance, validated_data):
        ...
        # Handle clearing hero_image when empty string is provided
        if 'hero_image' in validated_data and validated_data['hero_image'] == '':
            validated_data['hero_image'] = None
        
        # Generate unique slug from title if title is being updated
    ...
    def get_article_tags(self, obj):
        return [{'id': tag.tag.id, 'name': tag.tag.name, 'slug': tag.tag.slug, 'type': tag.tag.type} for tag in obj.article_tags.all()]
    ...
    def get_compare_extra(self, obj):
        if hasattr(obj, 'compare_extra'):
            return {
                ...
                'right_product': ProductSerializer(obj.compare_extra.right_product).data,
                'rounds': obj.compare_extra.rounds,
                'winner_product': ProductSerializer(obj.compare_extra.winner_product).data if obj.compare_extra.winner_product else None,
            }
        return None
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Comment
        fields = ['id', 'article', 'article_detail', 'article_id', 'user', 'content', 'status', 'author_name', 'author_email', 'parent', 'replies', 'helpful_count', 'article_title', 'created_at']
        read_only_fields = ['id', 'created_at']

    def create(self, validated_data):
        # Ensure article is provided for creation
        if 'article' not in validated_data:
            raise serializers.ValidationError({'article': 'This field is required for comment creation.'})
        return super().create(validated_data)
    ...
    def get_article_detail(self, obj):
        if obj.article:
            # Check if slug is valid (not empty, not None, not 'None' string)
...
class UserReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserReview
        fields = ['id', 'product', 'product_id', 'user', 'rating', 'title', 'content', 'pros', 'cons', 'is_verified', 'is_helpful', 'status', 'created_at']
        read_only_fields = ['id', 'user', 'created_at']

    def create(self, validated_data):
        # product_id'yi product field'ına map et (eğer varsa)
...
class AffiliateLinkSerializer(serializers.ModelSerializer):
    class Meta:
        model = AffiliateLink
        fields = ['id', 'product', 'merchant', 'url_template', 'active', 'created_at']
        read_only_fields = ['id', 'created_at']
...
class OutboundClickSerializer(serializers.ModelSerializer):
    class Meta:
        model = OutboundClick
        fields = ['id', 'product', 'article', 'user', 'merchant', 'ip', 'user_agent', 'created_at']
        read_only_fields = ['id', 'created_at']
...
class ProductComparisonSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'left_product', 'right_product', 'title', 'description', 'features', 'winner', 'is_public', 'created_at']
        read_only_fields = ['id', 'created_at']
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['username', 'email', 'password', 'password_confirm', 'first_name', 'last_name', 'marketing_emails', 'push_notifications', 'email_notifications']

    def validate_email(self, value):
        if User.objects.filter(email=value).exists():
            raise serializers.ValidationError("Bu e-posta adresi daha önce kullanılmış.")
        return value
...
class ArticleViewSerializer(serializers.ModelSerializer):
    class Meta:
        model = ArticleView
        fields = ['id', 'article', 'user', 'ip_address', 'user_agent', 'referer', 'created_at']
        read_only_fields = ['id', 'created_at']
...
class AffiliateLinkSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AffiliateLink
        fields = ['id', 'product', 'product_name', 'merchant', 'url_template', 'active', 'created_at', 'updated_at']
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class MonthlyAnalyticsSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'year', 'month', 'total_views', 'total_affiliate_clicks', 'total_users', 'created_at', 'updated_at']
        read_only_fields = ['id', 'created_at', 'updated_at']
