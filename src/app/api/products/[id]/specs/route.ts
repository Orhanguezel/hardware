...
export async function GET(
  ...
  try {
    ...
    if (isNumeric) {
      ...
      apiUrl = `${DJANGO_API_URL}/products/${id}/specs/`
    } else {
      // If it's not a number, treat it as slug
      apiUrl = `${DJANGO_API_URL}/products/slug/${id}/specs/`
    }
...
export async function POST(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: errorData.error || 'Failed to create spec' },
        { status: response.status }
      )
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to create spec' },
      { status: 500 }
    )
  }
}
