...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = queryString
      ? `${DJANGO_API_URL}/categories/?${queryString}`
      : `${DJANGO_API_URL}/categories/`
...
export async function POST(request: NextRequest) {
  try {
    ...
    // accessToken'ı type-safe olmayan şekilde ayrı değişkene al
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: (errorData as any).error || 'Failed to create category',
        },
