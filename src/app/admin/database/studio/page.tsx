...
import { ArrowLeft, ExternalLink, RefreshCw, Maximize2, Minimize2, Database, Table, Users, Package, FolderTree, Star, MessageSquare, Tag, Settings } from 'lucide-react'
...
export default function DatabaseStudioPage() {
  ...
  return (
    <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-background' : 'space-y-6'}`}>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              {React.createElement(getTableIcon(table), { className: "w-5 h-5" })}
              {table} Tablosu
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex gap-2">
              <Button asChild>
                <a href={`${adminUrl}/${getModelName(table)}/`} target="_blank" rel="noopener noreferrer">
                  <Table className="w-4 h-4 mr-2" />
                  Tabloyu Görüntüle
                </a>
              </Button>
              <Button asChild variant="outline">
                <a href={`${adminUrl}/${getModelName(table)}/add/`} target="_blank" rel="noopener noreferrer">
      ...
      <Card className={`${isFullscreen ? 'h-[calc(100vh-80px)]' : 'h-[600px]'}`}>
        <CardContent className="p-8 h-full flex flex-col items-center justify-center">
          <div className="text-center max-w-2xl">
            ...
            <p className="text-muted-foreground mb-6">
              Django Admin Panel güvenlik politikaları nedeniyle iframe içinde yüklenemiyor. 
              Aşağıdaki butonları kullanarak yeni sekmede açabilirsiniz.
            </p>
            ...
            <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <h4 className="font-semibold mb-2 text-blue-900 dark:text-blue-100">Güvenlik Notu</h4>
              <p className="text-sm text-blue-700 dark:text-blue-300">
                Django Admin Panel güvenlik nedeniyle iframe içinde yüklenemiyor. 
                Bu, verilerinizin güvenliğini artırır.
              </p>
            </div>
          </div>
        </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  ...
                  <li>• Kayıtları silmek için checkbox seçip "Delete selected" kullanın</li>
