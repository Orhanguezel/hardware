...
export function Header() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        if (result.success) {
          // Only show parent categories (categories without parent)
  ...
  useEffect(() => {
    const searchTimeout = setTimeout(async () => {
      if (searchQuery.trim().length >= 2) {
        ...
        try {
          ...
          const result = await response.json()
          if (result.success) {
            // Combine articles, products and users into a single array with type indicators
            const combinedResults = [
              ...(result.data.articles || []).map((item: any) => ({ ...item, type: 'article' })),
              ...(result.data.products || []).map((item: any) => ({ ...item, type: 'product' })),
              ...(result.data.users || []).map((item: any) => ({ 
                ...item, 
                type: 'user',
                name: item.name || `${item.first_name || ''} ${item.last_name || ''}`.trim() || item.username
              }))
            ]
    ...
    }, 200) // Faster response time like Spotlight

    return () => clearTimeout(searchTimeout)
  }, [searchQuery])

  // Keyboard navigation (Spotlight-like)
  useEffect(() => {
      ...
      switch (event.key) {
        ...
        case 'Enter':
          ...
          if (selectedIndex >= 0 && selectedIndex < searchResults.length) {
            ...
            const url = selectedItem.type === 'article' 
              ? `/reviews/${selectedItem.slug}` 
              : `/products/by-slug/${selectedItem.slug}`
            router.push(url)
            setShowSearch(false)
            setSearchQuery('')
            setShowResults(false)
    ...
    <>
                  ...
                  <div className="max-h-96 overflow-y-auto border-t">
                    <div className="space-y-1 p-2">
                      {searchResults.map((item: any, index: number) => {
                        ...
                        const Icon = item.type === 'article' ? FileText : item.type === 'user' ? User : Package
                        ...
                        const url = item.type === 'article' ? getArticleUrl(item) : 
                                   item.type === 'user' ? `/users/${item.id}` : 
                                   `/products/by-slug/${item.slug}`
                        const title = item.type === 'article' ? item.title : 
                                    item.type === 'user' ? item.name : 
                                    `${item.brand} ${item.model}`
                        const subtitle = item.type === 'article' ? item.excerpt : 
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  >
                                    {item.type === 'article' ? 'Makale' : 
                                     item.type === 'user' ? (item.role === 'ADMIN' ? 'Admin' : 'Kullanıcı') : 
                                     'Ürün'}
                                  </Badge>
                                  {item.category && (
      ...
      <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          <Link href="/compare-articles" className="text-sm font-medium hover:text-primary">
            Karşılaştırma
          </Link>
        ...
        <div className="md:hidden border-t bg-background">
          <nav className="container py-4 space-y-4">
            ...
            <div className="border-t pt-4">
              <div className="text-sm font-medium text-muted-foreground mb-2">Kategoriler</div>
              {isCategoriesLoading ? (
                ...
                <div className="text-sm text-muted-foreground ml-2">Henüz kategori bulunmuyor</div>
              )}
            </div>
            
            {/* Mobile Admin Link */}
            {session && ((session.user as any)?.role === 'SUPER_ADMIN' || (session.user as any)?.role === 'ADMIN') && (
              <div className="border-t pt-4">
                <Link 
                  href="/admin" 
